...
//    open var prefix: String = ""
    ...
    open var maxLength: Int {
        get {
            return formatPattern.count > 0 ? formatPattern.count : Int.max
        }
    }
    ...
    open override var textInputMode: UITextInputMode? {
            guard let prefix = languagePrefix else {
                return super.textInputMode
            }
            return UITextInputMode.activeInputModes.first(where: {
                $0.primaryLanguage?.contains(prefix) == true
            })
        }
    ...
    open func formatText() {
        ...
        if self.maxLength > 0 {
            ...
            if finalText.count > 0 {
                super.text = finalText//"\(self.prefix)\(finalText)"
            } else {
